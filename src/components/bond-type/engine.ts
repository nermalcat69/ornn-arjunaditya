...
export class BondType {
  ...
  private layout() {
    ...
    LINES.forEach((word, li) => {
      ...
      POSES.forEach((pose) => {
        ...
        const dys = pose.dy[li];
        // ponytail: poses are authored for 5-letter lines; longer words reuse
        // the last gap/dy so a name swap doesn't need new pose tables.
        const span = ls.reduce(
          (a, _l, i) => (i === 0 ? 0 : a + (gaps[i - 1] ?? last(gaps))),
          0,
        ) * H;
        let x = typesetCenter + pose.shift[li] * H - span / 2;
        ls.forEach((l, i) => {
          if (i > 0) x += (gaps[i - 1] ?? last(gaps)) * H;
          l.px.push(x - cx(l));
          l.py.push((dys[i] ?? last(dys)) * H);
        });
      });
    });
  }
  ...
  private offsetAt(l: Letter, i: number, t: number): [number, number] {
    ...
    const lastPose = this.seq[moves - 1];
    return [l.px[lastPose] * (1 - p), l.py[lastPose] * (1 - p)];
  }
